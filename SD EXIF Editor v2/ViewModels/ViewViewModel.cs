            ...
            private async void LoadItemsCivitItemViewModels()
            {
                ...
                var factory = LoggerFactory.Create(builder => { builder.AddNLog(); });
                    ...
                    if (Metadata.Model is SDModel model)
                    {
                        ...
                        CivitItemViewModels.Add(new CivitItemViewModel(await _civitService.GetItemFromHash(model.Name, model.Hash, null), _settingsService, factory.CreateLogger<CivitItemViewModel>()));
                    }

                    foreach (var lora in Metadata.Loras)
                    {
                        _logger.LogDebug($"Loading CivitItemViewModel for lora: {lora.Name}");
                        CivitItemViewModels.Add(new CivitItemViewModel(await _civitService.GetItemFromHash(lora.Name, lora.Hash, lora.Strength), _settingsService, factory.CreateLogger<CivitItemViewModel>()));
                    }
